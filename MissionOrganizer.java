...
public class MissionOrganizer implements Iterable<Mission>
{
	...
	ArrayList<Mission> cheapMission = new ArrayList<>();
	TreeSet expensiveMission = new TreeSet<>();
	
	MissionOrganizer()
	{
		this.cheapMission = cheapMission;
		this.expensiveMission = expensiveMission;
	...
	public void add(Mission mission)
	{
		if (mission.getCost() < ONE_TRILLION) {
			if (!cheapMission.contains(mission)) {
		...
		if (mission.getCost() >= ONE_TRILLION) {
			if (!expensiveMission.contains(mission)) {
	...
	public Iterator<Mission> iterator()
	{
		...
		al.addAll(cheapMission);
		al.addAll(expensiveMission);
...
//        new Mission("Sirius", 1E16f),
//        new Mission("Alpha Centauri", 8E11f),
//        new Mission("Wolf", 1E16f),
//        new Mission("Lalande", 8E11f),
	};
